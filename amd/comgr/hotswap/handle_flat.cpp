...
HandlerResult handleFLAT(RaiseContext &ctx, const DecodedInst &di,
  ...
  // CanonicalOp doc block in `canonical_op.hpp` for the correctness argument and
  ...
  if (sop == CanonicalOp::GLOBAL_LOAD_ASYNC_TO_LDS_B8 ||
    ...
    // emulation.  See the CanonicalOp doc block in `canonical_op.hpp` for the
  ...
  // `canonical_op.hpp` for the design rationale.
