...
; RUN:     --emit-ir=s_cbranch_vcc_wave_mask_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=VCC
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx950 \
; RUN:     --emit-ir=s_cbranch_vccnz_wave_mask_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=VCCNZ
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx950 \
; RUN:     --emit-ir=sop2_wave_mask_scc_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=SCC
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx950 \
; RUN:     --emit-ir=sop2_wave_mask_exec_scc_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=EXEC
; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx950 \
; RUN:     --emit-ir=sop2_wave_mask_scalar_scc_kernel 2>/dev/null \
...
; Scalar mask algebra that preserves a per-lane shadow must derive SCC from
; the whole shadow ballot, not from the source-width scalar bits.
;
...
; Complementing forms keep scalar SCC because their complemented register bits
; can affect SCC outside the modeled per-lane mask. Their per-lane shadows still
; propagate for later mask consumers.
;
