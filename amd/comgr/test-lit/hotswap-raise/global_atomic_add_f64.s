...
;
; Lift test for global_atomic_add_f64. Native on gfx90a/gfx940-family
; (pseudo `GLOBAL_ATOMIC_ADD_F64`). Same lift shape as the FLAT
; variant but the address space resolves to addrspace(1) when the
; backend re-lowers, because the raiser uses the global-store addr
; decoder.
