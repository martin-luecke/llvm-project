...
; RUN:   | %FileCheck %s --check-prefix=CHECK-PERM
;
...
; It is the forward (scatter) permute — the inverse of bpermute.
;
...
; test does not exercise cross-wave — see ds_bpermute_b32_wave32_rebase.s.
...
; CHECK-PERM-LABEL: define amdgpu_kernel void @ds_permute_b32_kernel(
	s_load_dword s3, s[0:1], 0x24
	s_load_dword s4, s[0:1], 0x10
	s_waitcnt lgkmcnt(0)
	s_and_b32 s3, s3, 0xffff
...
; CHECK-PERM: %perm = call i32 @llvm.amdgcn.ds.permute(i32 %{{[^,]+}}, i32 %{{[^,]+}})
	s_waitcnt lgkmcnt(0)
	v_add_f32_e32 v2, v2, v3
	v_lshl_add_u64 v[0:1], s[2:3], 0, v[0:1]
	global_store_dword v[0:1], v2, off
.Lpermute_end:
	s_endpgm
; CHECK-PERM: declare {{.*}}i32 @llvm.amdgcn.ds.permute(i32, i32)
	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
