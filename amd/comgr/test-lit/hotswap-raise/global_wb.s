...
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=global_wb_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=IR
; RUN: %raise_cli %t.hsaco --target-isa=gfx942 --write-hsaco=%t.gfx942.hsaco 2>/dev/null \
...
; RUN:   && %raise_cli %t.sys.hsaco --target-isa=gfx942 --emit-ir=global_wb_kernel 2>/dev/null \
; RUN:   | %FileCheck %s --check-prefix=IR-SYS
; RUN: %raise_cli %t.sys.hsaco --target-isa=gfx942 --write-hsaco=%t.sys.gfx942.hsaco 2>/dev/null \
...
; RUN:   && %raise_cli %t.cu.hsaco --target-isa=gfx942 --emit-ir=global_wb_kernel 2>/dev/null \
...
; system-scope writeback. CU scope is a manual-defined no-op/done-returning
; form. SE scope has no matching LLVM/gfx942 fence scope and must refuse rather
; than widen silently.
...
; REFUSE-SE: failed to raise: global_wb [FLAT]
