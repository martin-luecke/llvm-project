...
; RUN:     --emit-ir=div_carry_sgpr_kernel \
; RUN:   | %FileCheck %s
;
...
; The carry is per-lane, so the s_mov must commit the producer's per-lane
; shadow into VCC rather than re-widening the truncated SGPR. See
; hotswap/docs/sgpr-wave-mask-translation.md.
