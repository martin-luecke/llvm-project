...
; RUN:   | %FileCheck %s
;
; Qwen's boolean-mask index_put object places many related kernels in one
; `.text` section. The selected OpaqueType<8> index_put symbol ends with a
; backward branch and no trailing `s_endpgm`; a later, different put/take symbol
; contains `global_atomic_cmpswap_b32`. Decoding must stop at the selected
; symbol's ELF size, otherwise obstruction analysis reports the later symbol's
; CAS as if it belonged to the selected index_put kernel.
;
