...
; RUN:   && raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=v_add_min_u32_kernel 2>/dev/null | %FileCheck %s --check-prefixes=BOTH,DEFAULT
; RUN: raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=v_add_min_u32_clamp_kernel 2>/dev/null | %FileCheck %s --check-prefixes=BOTH,CLAMP
;
; Lift test for gfx1250 v_add_min_u32:
...
; DEFAULT: %v_add_min_u32_sum{{[0-9]*}} = call i32 @llvm.uadd.sat.i32(i32 %{{[^,]+}}, i32 -1)
; CLAMP: %v_add_min_u32_sum{{[0-9]*}} = call i32 @llvm.uadd.sat.i32(i32 {{[^,]+}}, i32 {{[^,]+}})
; DEFAULT: %v_add_min_u32{{[0-9]*}} = call i32 @llvm.umin.i32(i32 %v_add_min_u32_sum{{[0-9]*}}, i32 %{{[^)]+}})
; CLAMP: %v_add_min_u32{{[0-9]*}} = call i32 @llvm.umin.i32(i32 %v_add_min_u32_sum{{[0-9]*}}, i32 3)
; BOTH-NOT: call {{.*}}@llvm.amdgcn.add.min.u32
;
