...
; reads source within-row (W - N) mod 16, computed as (W + (16 - N)) & 15;
; for N=1 that is (W + 15) & 15.  The wrap keeps the source lane inside the
; 16-lane row, so InRange is the constant true (wave-size-oblivious).
