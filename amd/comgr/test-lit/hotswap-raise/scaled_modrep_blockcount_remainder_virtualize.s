...
; scaled x extent, so every in-kernel x size query must be halved back to the
; source size. This kernel reads hidden_block_count_x and hidden_remainder_x,
; both of which the raiser derives from the dispatch grid-size and group-size
; reads: block_count = grid/group, remainder = grid%group. Each underlying x
; read is virtualized (halved) before the div/rem, so the source kernel observes
; the un-scaled counts. This complements scaled_modrep_wgsize_virtualize.s,
; which only covers the group-size read. See
...
; the grid-size and group-size reads feeding the derived counts are halved:
; CHECK-DAG: %source_hidden_grid_size_0_dd_virt{{.*}} = lshr i32 {{.+}}, 1
; CHECK-DAG: %source_hidden_wg_size_0_dd_virt{{.*}} = lshr i32 {{.+}}, 1
; block_count_x = grid/group and remainder_x = grid%group over the halved reads:
...
; block_count_x lives at kernarg offset 0x8, remainder_x at 0xc:
