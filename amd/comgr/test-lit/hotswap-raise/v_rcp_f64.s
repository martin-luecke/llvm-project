...
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx942 --emit-ir=v_rcp_f64_kernel \
; RUN:   | %FileCheck %s
...
; CHECK: call double @llvm.amdgcn.rcp.f64(double [[NEG]])
        v_rcp_f64_e64 v[2:3], -v[0:1]
; CHECK: [[ABS:%[^ ]+]] = call double @llvm.fabs.f64(double %{{.+}})
; CHECK: call double @llvm.amdgcn.rcp.f64(double [[ABS]])
        v_rcp_f64_e64 v[4:5], |v[0:1]|
