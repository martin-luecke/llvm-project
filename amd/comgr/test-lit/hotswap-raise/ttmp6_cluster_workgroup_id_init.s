; RUN: %llvm_mc -mcpu=gfx1250 %s -o %t.o && %ld_lld -shared %t.o -o %t.hsaco \
; RUN:   && %raise_cli %t.hsaco --target-isa=gfx950 --emit-ir=ttmp6_cluster_workgroup_id_init_kernel 2>/dev/null \
; RUN:   | %FileCheck %s
;
; gfx12+ source kernels may read ttmp6 cluster workgroup fields. The target
; execution model used by HotSwap has no source clusters, so the singleton
; cluster encoding is zero. Leaving ttmp6 undef corrupts workgroup/address math.
;
; CHECK-LABEL: define amdgpu_kernel void @ttmp6_cluster_workgroup_id_init_kernel(
; CHECK: store i32 0, ptr %ttmp_6
; CHECK: and i32 %{{[^,]+}}, 15

	.amdgcn_target "amdgcn-amd-amdhsa--gfx1250"
	.amdhsa_code_object_version 6
	.text
	.globl	ttmp6_cluster_workgroup_id_init_kernel
	.p2align	8
	.type	ttmp6_cluster_workgroup_id_init_kernel,@function
ttmp6_cluster_workgroup_id_init_kernel:
	s_load_b64 s[0:1], s[0:1], 0x0
	s_and_b32 s2, ttmp6, 15
	s_wait_loadcnt 0x0
	v_mov_b32_e32 v1, s2
	global_store_b32 v0, v1, s[0:1] scale_offset
	s_endpgm

	.section	.rodata,"a",@progbits
	.p2align	6, 0x0
	.amdhsa_kernel ttmp6_cluster_workgroup_id_init_kernel
		.amdhsa_kernarg_size 8
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_wavefront_size32 1
		.amdhsa_next_free_vgpr 2
		.amdhsa_next_free_sgpr 3
		.amdhsa_float_denorm_mode_32 3
		.amdhsa_inst_pref_size 1
	.end_amdhsa_kernel
	.text
	.amdgpu_metadata
---
amdhsa.kernels:
  - .args:
      - { .address_space:  global, .offset: 0, .size: 8, .value_kind: global_buffer }
    .group_segment_fixed_size: 0
    .kernarg_segment_align: 8
    .kernarg_segment_size: 8
    .max_flat_workgroup_size: 1024
    .name: ttmp6_cluster_workgroup_id_init_kernel
    .private_segment_fixed_size: 0
    .sgpr_count: 3
    .symbol: ttmp6_cluster_workgroup_id_init_kernel.kd
    .vgpr_count: 2
    .wavefront_size: 32
amdhsa.version: [1, 2]
...
	.end_amdgpu_metadata
