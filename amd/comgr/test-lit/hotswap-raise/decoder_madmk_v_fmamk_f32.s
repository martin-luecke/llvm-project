...
;
; The decoder fix landed after the GPT-OSS `attn_fwd` corpus kernel
; hit the strict-check abort. This
...
; The K=pi literal (encoding 0x40490FDB) lands at the middle operand
...
; literal appears SOMEWHERE in the IR, allowing for any of LLVM
; IR's valid f32-constant renderings:
;   * decimal/hex-int form like `0x40490FDB` or `3.141593e+00`
;   * f-prefixed typed-float hex form like `f0x40490FDB` (LLVM's
;     newer disambiguator that distinguishes a float constant from
;     an integer literal that happens to share its bit pattern)
...
;
; Match the FMA call with the K=pi literal at the middle (second)
; operand slot — that's where the V_FMAMK_F32 handler routes the
; K-imm per its `srcF(0)=src0, srcF(1)=K, srcF(2)=src2` convention
; (handle_valu.cpp). The literal can render in any of LLVM IR's
; valid f32-constant forms:
;   * hex-int form like `0x40490FDB`
;   * f-prefixed typed-float hex form like `f0x40490FDB` (newer
;     LLVM disambiguates float constants from integer hex of the
;     same bit pattern)
...
; test to the IR printer's heuristic; matching any of the four
