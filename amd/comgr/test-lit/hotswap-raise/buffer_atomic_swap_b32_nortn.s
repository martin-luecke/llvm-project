...
; The atomic itself — same raw-buffer shape as the RTN variant.
; CHECK: call i32 @llvm.amdgcn.raw.buffer.atomic.swap

; Negative pin for the write-back.  On RTN the handler writes the
; intrinsic result back to `op.dst()` via writeReg32, which in IR
; surfaces as an `%atomic_result` having an SSA USER that stores
; into a tied VGPR.  On non-RTN we want the intrinsic RESULT to be
...
; intrinsic — the `CHECK-NEXT` pin catches the usual writeReg32
