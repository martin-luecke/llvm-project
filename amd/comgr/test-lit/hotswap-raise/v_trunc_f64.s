...
; CHECK-LABEL: define amdgpu_kernel void @v_trunc_f64_kernel(
v_trunc_f64_kernel:
; CHECK: call double @llvm.trunc.f64(double {{.*}})
        v_trunc_f64 v[0:1], v[0:1]
; CHECK: [[NEG:%[^ ]+]] = fneg double %{{.+}}
; CHECK: call double @llvm.trunc.f64(double [[NEG]])
        v_trunc_f64_e64 v[2:3], -v[0:1]
; CHECK: [[ABS:%[^ ]+]] = call double @llvm.fabs.f64(double %{{.+}})
; CHECK: call double @llvm.trunc.f64(double [[ABS]])
        v_trunc_f64_e64 v[4:5], |v[0:1]|
; CHECK: declare {{.*}}double @llvm.trunc.f64(double)
        s_endpgm
