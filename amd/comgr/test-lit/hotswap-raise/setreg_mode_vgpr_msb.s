...
; RUN:     --emit-ir=setreg_mode_vgpr_msb_kernel 2>&1 \
; RUN:   | %FileCheck %s
;
; Regression guard for a gfx1250 kernel-prologue MODE setreg that
; co-encodes VGPR_MSB in imm[12:19].  The compiler folds VGPR_MSB into
; the REPLAY_MODE setreg producing values like 0x1001 (bit 0 = REPLAY_MODE,
; bits 12..19 = MSB bank selector). The hardware latches bits[12:19]
; unconditionally, independent of the (offset, size) field selector.
;
; The kernel: MODE setreg with imm 0x1001 sets DST_VGPR_MSB=01 (+256 to
; dst encodings), so the following writelane to the encoded v0 maps to
; physical v256. A subsequent encoded-v0 read must source v256, not v0.
; The final global store keeps the Vgpr256 def-use chain alive through DCE.

; CHECK-LABEL: define amdgpu_kernel void @setreg_mode_vgpr_msb_kernel(
; writelane target resolves to Vgpr256 after applying DST_VGPR_MSB=01.
