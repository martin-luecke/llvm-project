...
; RUN: %raise_cli %t.hsaco --target-isa=gfx1250 --emit-ir=v_mad_u16_opsel_kernel 2>/dev/null \
...
; CLAMP: %mad_u16_clamp{{[0-9]*}} = call i32 @llvm.umin.i32(i32 %mad_u16_wide{{[0-9]*}}, i32 65535)
; CLAMP: trunc i32 %mad_u16_clamp{{[0-9]*}} to i16
;
; op_sel:[1,1,1,1] selects the high i16 of each source and writes the high
; half of dst: each source is extracted via lshr 16, and the result merges
; back through shl 16 (merge_hi) preserving the low half.
; OPSEL-LABEL: define amdgpu_kernel void @v_mad_u16_opsel_kernel(
; OPSEL: lshr i32 %{{[^,]+}}, 16
; OPSEL: %mad_u16_mul{{[0-9]*}} = mul i16
; OPSEL: %mad_u16{{[0-9]*}} = add i16 %mad_u16_mul{{[0-9]*}},
; OPSEL: shl i32 %{{[^,]+}}, 16
; OPSEL: %mad_u16_merge_hi{{[0-9]*}} = or i32
...
v_mad_u16_clamp_kernel:
	...
	.globl	v_mad_u16_opsel_kernel
	.p2align	8
	.type	v_mad_u16_opsel_kernel,@function
v_mad_u16_opsel_kernel:
	s_mov_b64 s[6:7], s[0:1]
	s_load_b128 s[0:3], s[6:7], 0x0
	s_load_b96 s[4:6], s[6:7], 0x10
	s_wait_loadcnt 0x0
	...
	v_mov_b32_e32 v1, s5
	v_mov_b32_e32 v2, s6
	v_mad_u16 v3, v0, v1, v2 op_sel:[1,1,1,1]
	v_mov_b32_e32 v4, 0
	global_store_b32 v4, v3, s[0:1] scale_offset
	s_endpgm
	...
	.end_amdhsa_kernel
	.p2align	6, 0x0
	.amdhsa_kernel v_mad_u16_opsel_kernel
		.amdhsa_kernarg_size 28
		.amdhsa_user_sgpr_count 2
		.amdhsa_user_sgpr_kernarg_segment_ptr 1
		.amdhsa_wavefront_size32 1
...
amdhsa.kernels:
  ...
  - .args:
    ...
    .kernarg_segment_size: 28
    .max_flat_workgroup_size: 1024
    .name:           v_mad_u16_opsel_kernel
    .private_segment_fixed_size: 0
    .sgpr_count:     8
    .symbol:         v_mad_u16_opsel_kernel.kd
    .vgpr_count:     5
    .wavefront_size: 32
amdhsa.version: [1, 2]
...
