...
void driftCheckSrcN(DecodedInst &Di, const MCInstrDesc &Desc) {
  ...
  // strict srcN-position check at k=0 for this family. There is currently no
  // movrel handler, so the opcode still fails gracefully downstream (and is
  // stubbed under HSA_HOTSWAP_STUB_FAILED_KERNELS) instead of hard-aborting the
  // whole process here via report_fatal_error.
