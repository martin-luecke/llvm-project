...
bool isIntrinsicVGPRSafePropagator(Intrinsic::ID Id) {
  ...
  case Intrinsic::sqrt:
  // Floating max/min families lower as per-lane VGPR-only operations.  The
  // scalar NUM handlers use maximumNumber/minimumNumber, and the vector IEEE
  // handlers use maximum/minimum; neither shape forces an SGPR operand.
