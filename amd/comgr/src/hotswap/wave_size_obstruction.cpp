//===- wave_size_obstruction.cpp - Hotswap transpiler ---------------------===//
//
...
ObstructionReport buildObstructionReport(ArrayRef<DecodedInst> Insts,
  ...
  // obstruction triggers should be added by extending canonical_op.h +
  ...
  for (const DecodedInst &Di : Insts) {
    ...
    if (sop == CanonicalOp::V_READLANE_B32 || sop == CanonicalOp::V_WRITELANE_B32) {
      ...
      // tracked in wave_size_obstruction.h's TODO block.
      continue;
    }
    ...
    // non-commutative atomics modeled in canonical_op.h today. New
  ...
  // wave_size_obstruction.h's TODO block.
