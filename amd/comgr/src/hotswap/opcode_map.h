//===- opcode_map.h - Hotswap transpiler ----------------------------------===//
//
...
// float) into a side table -- see `VCmpMeta` above. That keeps the CanonicalOp
...
public:
  ...
  // `CanonicalOp::V_CMP` or `CanonicalOp::V_CMPX`.
...
private:
  ...
  llvm::DenseMap<unsigned, VCmpMeta> Vcmp;
};
