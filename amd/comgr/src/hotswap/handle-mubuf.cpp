...
HandlerResult handleMUBUF(RaiseContext &Ctx, const DecodedInst &Di,
  ...
  if (Sop >= CanonicalOp::BUFFER_ATOMIC_ADD && Sop <= CanonicalOp::BUFFER_ATOMIC_MAX_F64) {
    ...
    // BUFFER_ATOMIC_{MIN,MAX}_F64: hand-rolled CAS loop. The single
    // LLVM pseudo `BUFFER_ATOMIC_{MIN,MAX}_F64` covers two genuinely
    // different HW operations:
    //   * gfx942 (`buffer_atomic_min_f64` / `_max_f64`, ISA manual
    //     12.15.3 op 80/81): raw `src < tmp ? src : tmp` /
    //     `src > tmp ? src : tmp`. No NaN handling, no +0/-0 tiebreak,
    //     asymmetric on NaN inputs.
    //   * gfx1250 (`buffer_atomic_min_num_f64` / `_max_num_f64`, sp3
    //     manual sec 8.7.17/8.7.23): IEEE 754-2019 minimumNumber /
    //     maximumNumber. qNaN-quieting, sNaN -> quieted, +0 > -0.
    // No single LLVM intrinsic captures both: the available
    // `raw_buffer_atomic_fmin/fmax.f64` carries IEEE 754-2008
    // minNum/maxNum semantics, and there is no fminimumnum/fmaximumnum
    // variant; `atomicrmw fminimumnum/fmaximumnum` on buffer fat
    // pointers is hard-rejected by AMDGPULowerBufferFatPointers.cpp.
    // The CAS loop lets us pick the exact comparator per source ISA
    // (raw `fcmp olt/ogt` + `select` for gfx942, `llvm.minimumnum/
    // maximumnum.f64` for gfx1250) while preserving SRD-relative
    // addressing via `raw_buffer_atomic_cmpswap.i64`. Disambiguation
    // keys off the `_num_` substring of the assembled mnemonic since
    // the LLVM pseudo name is shared across both subtargets.
    if (Sop == CanonicalOp::BUFFER_ATOMIC_MIN_F64 ||
      ...
      Function *CasFn = Intrinsic::getOrInsertDeclaration(
          ...
          {Ctx.I64Ty});
      Ctx.emitUnderExec([&] {
        // Initial load: read the current memory value as i64 (the
        // CAS will reload on retry, so this is the loop seed).
        ...
        if (IsIeeeNum) {
          ...
          // Raw gfx942 semantics: `src CMP tmp ? src : tmp` (i.e.
          // src wins iff the comparison is true; tmp wins on
          // equal, on tie, and on every NaN-involving compare since
          // `fcmp ogt`/`olt` returns false for unordered inputs).
          Value *Cmp =
        ...
        Value *Returned = Ctx.B.CreateCall(
            ...
            {NewI64, Expected, Mbuf.Srd, Mbuf.Voffset, Mbuf.Soffset,
             Mbuf.AuxFlags},
            "fp64_minmax_cas");
        // Retry on conflict; exit when our CAS won (returned ==
        // expected means no concurrent writer mutated the slot
        // between our load and our cmpswap).
        ...
        // RTN form: publish the pre-modification memory value to
        // the dst VGPR pair. The first iteration's `Returned` is
        // the genuine pre-op value; later iterations only land here
        // when the CAS finally won, at which point `Returned`
        // equals the post-load-pre-CAS memory value as seen by the
        // winning attempt -- still the correct "pre-this-atomic"
        // observation for the lane.
        if (Di.NumDefs > 0)
    ...
    case CanonicalOp::BUFFER_ATOMIC_ADD_F64:
      ...
      IsFp = true;
      break;
    // BUFFER_ATOMIC_{MIN,MAX}_F64 are handled by the dedicated CAS-
    // loop block above this switch (see the comment there for the
    // per-source-ISA comparator selection). They never reach this
    // switch; this comment is only here as a navigation aid for
    // future readers.
    default:
