...
public:
  ...
  // EXEC alloca storage width chosen by the projection; the base returns the
  ...
  // raised IR runs on, not of the source ISA.
  ...
  //
  // Pure virtual so every new projection answers this explicitly; a silent
  // default could pick the wrong pass count for a widening projection.
...
public:
  ...
  // Both the phantom-lane widening regime and same-wave instantiations
  // carry exactly one source wave per target wave.
...
public:
  ...
  // `emitInitialExec` forces HW EXEC=-1 for the kernel body, so this projection
  // provides the full-wave-EXEC invariant.
  bool providesFullWaveExecInvariant() const override { return true; }
  // Wave32 -> wave64 widening stacks two source waves into one target
  // wave (lanes 0..31 and 32..63).
...
// translation relies on modulo-replication of an EXEC-manipulating kernel.
