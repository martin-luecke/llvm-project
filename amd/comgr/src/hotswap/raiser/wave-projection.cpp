...
// change that mapping override it.
// ----------------------------------------------------------------------------
...
Value *ModuloReplicationProjection::ballotI1ToWidth(IRBuilder<> &B, Value *Pred,
  ...
  if (WantedBits == WaveBits)
    return WaveMask;
  if (WantedBits < WaveBits)
    // Truncation is the modulo-replication projection of the target ballot
  ...
  // modulo-replication projection exists (the wider source wave has lanes
...
Value *ModuloReplicationProjection::extractLaneBitFromWaveMask(IRBuilder<> &B,
  ...
  if (SrcBits < DstBits) {
    ...
    // extending: modulo-replication's contract is that target lane L reads
...
Value *WaveNativeProjection::ballotI1ToWidth(IRBuilder<> &B, Value *Pred,
  ...
  // target-wave lifts (not our wave32->wave64 widening), so
