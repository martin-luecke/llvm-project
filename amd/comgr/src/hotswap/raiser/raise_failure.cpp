...
llvm::Error RaiseFailure::unsupportedInstructionForm(
    llvm::StringRef Mnemonic, uint64_t Offset, llvm::StringRef Format,
    const llvm::Twine &Detail) {
  return llvm::make_error<RaiseFailure>(
      RaiseFailureReason::UnsupportedInstructionForm, Mnemonic.str(),
      Format.str(), Offset, Detail.str());
}

llvm::Error RaiseFailure::unsupportedSourceHiddenArg(llvm::StringRef Mnemonic,
                                                     uint64_t Offset,
                                                     llvm::StringRef Format,
                                                     llvm::StringRef Detail) {
  return llvm::make_error<RaiseFailure>(
      RaiseFailureReason::UnsupportedSourceHiddenArg, Mnemonic.str(),
      Format.str(), Offset, Detail.str());
}

llvm::Error RaiseFailure::unsupportedOpcode(llvm::StringRef Mnemonic,
                                            uint64_t Offset,
                                            llvm::StringRef Format) {
  return llvm::make_error<RaiseFailure>(RaiseFailureReason::UnsupportedOpcode,
                                        Mnemonic.str(), Format.str(), Offset,
                                        std::string());
}

llvm::Error RaiseFailure::speUnsafeExecWriter(llvm::StringRef Mnemonic,
                                              uint64_t Offset,
                                              const llvm::Twine &Detail) {
  return llvm::make_error<RaiseFailure>(
      RaiseFailureReason::SPEUnsafeExecWriter, Mnemonic.str(),
      "SPE-unmodeled-EXEC-writer", Offset, Detail.str());
}
...
// ----------------------------------------------------------------------------
// Wave-size-obstruction factories: take the refused instruction for
// mnemonic / offset, and a kind-specific detail string.
// ----------------------------------------------------------------------------
...
llvm::Error makeCrossWaveFailure(RaiseFailureReason Reason,
  ...
  return llvm::make_error<RaiseFailure>(
      Reason, Mnemonic.str(), reasonString(Reason), Offset, KindDetail.str());
}

} // namespace

llvm::Error RaiseFailure::crossWaveLaneIdLeak(llvm::StringRef Mnemonic,
                                              uint64_t Offset,
                                              const llvm::Twine &KindDetail) {
  return makeCrossWaveFailure(RaiseFailureReason::CrossWaveLaneIdLeak, Mnemonic,
                              Offset, KindDetail);
}

llvm::Error RaiseFailure::crossWaveUnrewritableShuffle(
    llvm::StringRef Mnemonic, uint64_t Offset, const llvm::Twine &KindDetail) {
  return makeCrossWaveFailure(RaiseFailureReason::CrossWaveUnrewritableShuffle,
                              Mnemonic, Offset, KindDetail);
}

llvm::Error RaiseFailure::crossWaveShuffleRewritePending(
    llvm::StringRef Mnemonic, uint64_t Offset, const llvm::Twine &KindDetail) {
  return makeCrossWaveFailure(
      RaiseFailureReason::CrossWaveShuffleRewritePending, Mnemonic, Offset,
      KindDetail);
}

llvm::Error RaiseFailure::crossWaveReplicaRace(llvm::StringRef Mnemonic,
                                               uint64_t Offset,
                                               const llvm::Twine &KindDetail) {
  return makeCrossWaveFailure(RaiseFailureReason::CrossWaveReplicaRace,
                              Mnemonic, Offset, KindDetail);
}

llvm::Error RaiseFailure::crossWaveLanePredicatedExec(
    llvm::StringRef Mnemonic, uint64_t Offset, const llvm::Twine &KindDetail) {
...
llvm::Error RaiseFailure::strictUnsafeLowering(llvm::StringRef Mnemonic,
                                               uint64_t Offset,
                                               llvm::StringRef Site,
                                               llvm::StringRef Detail) {
  return llvm::make_error<RaiseFailure>(
      RaiseFailureReason::StrictUnsafeLowering, Mnemonic.str(), Site.str(),
      Offset, Detail.str());
}
