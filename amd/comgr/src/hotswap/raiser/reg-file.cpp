...
void AllocaRegFile::init(IRBuilder<> &B, Type *I32Ty, Type *I1Ty,
  ...
  Projection = &Proj;
  // EXEC storage width is chosen by the projection: source wave width for
  // modulo-replication, widened to the target mask for wave-native
  // widening so data-dependent EXEC writes survive the lift.
  ...
  // The initial EXEC value is projection-dependent: all-ones for
  // modulo-replication, and for wave-native widening a mask that
  // forces all target lanes active so the cross-lane pipeline runs even
  // on a partial-wave dispatch.
...
void AllocaRegFile::storeSGPR64(IRBuilder<> &B, int Idx, Value *V) {
  ...
  // Pair writes invalidate both halves of the shadow.
  if (OnSgprWritten) {
    OnSgprWritten(Idx);
    OnSgprWritten(Idx + 1);
  }
...
Value *AllocaRegFile::readReg32(IRBuilder<> &B, ParsedReg Pr) {
  ...
  // VCC as a 32-bit scalar read must go through the wave-mask ballot, not
  // a sign-extension of the local i1. Callers wanting a per-lane i1 call
  // `loadVCC` directly.
  ...
  if (Pr.RegKind == ParsedReg::EXEC) {
    ...
    // slice indexed by BaseIdx (LO/HI).
...
Value *AllocaRegFile::readReg64(IRBuilder<> &B, ParsedReg Pr) {
  ...
  if (Pr.RegKind == ParsedReg::EXEC_HI_SCRATCH)
    return B.CreateZExt(B.CreateLoad(B.getInt32Ty(), ExecHiScratch),
                        B.getInt64Ty());
  // VCC as a 64-bit scalar read must route through the wave-mask ballot:
  // a sign-extension of the local i1 would replicate the current lane's
  // VCC bit across all bits, a silent lie when the consumer expects a
  // wave-level mask (e.g. `s_and_b64 vcc, exec, vcc`).
  if (Pr.RegKind == ParsedReg::VCC)
  ...
  // TTMP[BaseIdx:BaseIdx+1] read as i64: combine the two adjacent i32
  // lanes the same way SGPR pairs are combined.
  if (Pr.RegKind == ParsedReg::TTMP && Pr.BaseIdx >= 0 &&
...
void AllocaRegFile::writeReg32(IRBuilder<> &B, ParsedReg Pr, Value *V) {
  ...
  if (Pr.RegKind == ParsedReg::EXEC) {
    ...
    // Default: preserve the other half's live value and merge. Under
    // wave-native widening an EXEC_LO write represents the whole
    // wave mask and is replicated into both halves of the widened EXEC.
    Value *V64 = B.CreateZExt(V, ExecTy);
    if (Projection && Projection->broadcastNarrowExecLoWrite() &&
        Pr.BaseIdx == 0) {
      // EXEC = (v << 32) | v: broadcast the whole-wave mask across both
      // halves of the widened EXEC.
...
void AllocaRegFile::writeRegExecWidth(IRBuilder<> &B, ParsedReg Pr, Value *V) {
  if (Pr.RegKind == ParsedReg::SGPR) {
    // SGPR storage is sized by the source architecture (32-bit on wave32
    // source, a 64-bit pair on wave64). Under wave-native widening
    // ExecTy is the wider target mask while the source-named SGPR stays
    // 32-bit, so the incoming EXEC-width value is narrowed to the source
