...
| DPP modifiers (any base VOP) (P5) | Decode lifts `dppCtrl` / `dppRowMask` / `dppBankMask` / `dppBoundCtrl` into `DecodedInst` before opcode canonicalisation. `emitUpdateDpp` emits `llvm.amdgcn.update.dpp.{i32,i64}`; `OpResolver::src*` routes src0 through `wrapDppIfNeeded`. DPP8 refuses (pending P5.b). Under cross-widening, `rewrite-cross-lane-divergent.cpp` lowers i32 `update.dpp` to a `ds_bpermute` selector for the intra-16-lane-row ctrl families (`quad_perm`, `row_shl:N`, `row_shr:N`, `row_ror:N`, `row_xmask:N`); wave-wide / cross-row ctrls still refuse (§5.3). | `2dc9aa927e`, `75cf67cc18` |
