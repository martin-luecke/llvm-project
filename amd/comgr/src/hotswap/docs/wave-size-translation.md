...
| `v_mbcnt_* -> v_cmp_*` / scalar mask algebra -> `s_*_saveexec_b32` under WaveNative (C4) | `v_mbcnt_lo` is source-wave-local, V_CMP/VCC/SGPR mask producers record target-width per-lane shadows, SOP mask algebra propagates those shadows, and SAVEEXEC refuses unless its source operand is read as an EXEC-width mask before routing the EXEC write through `storeExec`. Source-width fallback widening is not a proof for lane-position-sensitive masks. | `SaveExecFromLaneId` / `WaveNativeMbcntSaveExec` |
...
| `s_*_saveexec_b32` whose source mask is derived from `v_mbcnt_*` decoded-register provenance (C4) under a projection that does not preserve independent source-wave masks, or under WaveNative when the handler cannot prove the operand is an EXEC-width wave mask. This catches scalar EXEC masks built from lane-id data while allowing unrelated `v_mbcnt_*` shuffle selectors plus ordinary bounds masks. `v_cmpx` sites graduate directly under WaveNative; scalar saveexec sites additionally require the handler proof pinned by `mbcnt_saveexec_mask_refuse.s`. | `SaveExecFromLaneId` | `CrossWaveLanePredicatedExec` |
