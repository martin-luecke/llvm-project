//===- tdm-runtime.cpp - Hotswap transpiler -------------------------------===//
//
...
llvm::FunctionType *tdmHelperFnTy(llvm::LLVMContext &C) {
  ...
  // in the helper path. See `tdm-runtime.h` and the matching handler-side
  // elision in `handle-vimage.cpp`.
...
static llvm::Error linkTDMRuntimeImpl(llvm::Module &M,
...
llvm::Error linkTDMRuntime(llvm::Module &M, llvm::StringRef KernelName,
                           llvm::StringRef TargetIsa) {
  if (llvm::Error Err = linkTDMRuntimeImpl(M, TargetIsa))
    return RaiseFailure::deviceLibraryLinkFailed(
        KernelName, llvm::toString(std::move(Err)));
  return llvm::Error::success();
}
