...
#include "isa_profile.h"
#include "decoded_inst.h"
#include "parsed_reg.h"
...
#include "setpc_analysis.h"
#include "source_hidden_args.h"
#include "user_sgpr_layout.h"
#include "wave_projection.h"
#include "wave_size_obstruction.h"
#include "handlers.h"
#include "rewrite_cross_lane_divergent.h"
#include "c5_predicate_chain_classifier.h"
#include "tdm_runtime.h"
...
// parseReg, readOp32/64/ExecWidth, and OpResolver are in raise_context.h/cpp
// instructionWritesEXEC and the cross-wave gate live in wave_projection.h/cpp
// RaiseFailure + reasonString are in raise_failure.h/cpp
...
static RaiseResult raiseToIRImpl(llvm::ArrayRef<uint8_t> TextBytes,
  ...
  // `rsplit('-')` extracts the trailing component for validation; the
  // same trimmed name is later fed to `MCSubtargetInfo`, which only
  // accepts the bare processor name (passing the full canonical string
  // emits "is not a recognized processor for this target" and produces
  // a feature-less subtarget that aborts in `createMCDisassembler`).
  StringRef SourceCpu = SourceIsa.rsplit('-').second;
  if (SourceCpu.empty())
    SourceCpu = SourceIsa;
  // Strip any trailing `:sramecc±:xnack±` feature suffix that
  // `getElfIsaNameFromBuffer` may have appended; `parseArchAMDGCN` and
  // `createMCSubtargetInfo` both want just the GFX name.
  ...
  StringRef TargetCpu = CompilationTargetIsa;
  if (!TargetCpu.empty()) {
    TargetCpu = TargetCpu.rsplit('-').second;
    if (TargetCpu.empty())
      TargetCpu = CompilationTargetIsa;
    TargetCpu = TargetCpu.split(':').first;
  }
  ...
  if (!TargetCpu.empty()) {
    TargetSti = buildSubtargetInfo(*Mc.Target, TargetCpu);
  ...
  // See setpc_analysis.h + canonical_op.h's `S_SET_PC_I64` doc for the
  // analysis contract.
  ...
  for (size_t InstIdx = 0; InstIdx < Insts.size(); ++InstIdx) {
    ...
    // RaiseContext::resetLaneActiveCache in raise_context.h for the full
    ...
    if (Hr.Handled) {
      ...
      // See setpc_analysis.h + canonical_op.h's S_SET_PC_I64 doc +
    ...
    if (!PredReport.Refused && !PredReport.ObservedSites.empty()) {
      ...
      LLVM_DEBUG({
        dbgs() << "c5-predicate-chain: observed "
               ...
               << ProjectionName
               << " (refusal "
                  "suppressed per c5_predicate_chain_classifier.h "
                  "projection contract):\n";
