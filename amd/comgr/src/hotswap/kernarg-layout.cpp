...
SourceHiddenArgByte classifySourceHiddenArgByte(ArrayRef<KernelArgMeta> Args,
                                                int ByteOffset) {
  for (const KernelArgMeta &Arg : Args) {
    int ArgOffset = static_cast<int>(Arg.Offset);
    int ArgEnd = ArgOffset + static_cast<int>(Arg.Size);
    if (ByteOffset < ArgOffset || ByteOffset >= ArgEnd)
      continue;
