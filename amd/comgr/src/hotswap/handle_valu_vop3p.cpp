//===- handle_valu_vop3p.cpp - Hotswap transpiler -------------------------===//
//
...
#include "handle_valu_internal.h"

#include "canonical_op.h"
#include "wmma_lowering.h"
...
HandlerResult handleValuVoP3P(RaiseContext &Ctx, const DecodedInst &Di,
  ...
  case CanonicalOp::V_WMMA_F32_16x16x4_F32: {
    ...
    if (Ctx.TargetIsa.HasTensorOps) {
      ...
      // in `wave_projection.h` (`numSourceWavesPerTarget`,
  ...
  case CanonicalOp::V_WMMA_SCALE_F32_16x16x128_F8F6F4: {
    ...
    Function *WmmaFn = Intrinsic::getOrInsertDeclaration(
        ...
        {CdTy, ATy, BTy});
    Value *ResultVal = Ctx.B.CreateCall(WmmaFn, {
        MatrixAFmt, A,
        MatrixBFmt, B,
        CMod, C,
