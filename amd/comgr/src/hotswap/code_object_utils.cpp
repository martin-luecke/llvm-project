...
std::vector<std::string> listKernelNames(llvm::ArrayRef<uint8_t> ElfData) {
  ...
  auto DocOrErr =
      COMGR::elf_helpers::getElfMetadataDocument(toBufferRef(ElfData));
  if (!DocOrErr) {
    llvm::errs() << "transpiler: listKernelNames: "
                 << llvm::toString(DocOrErr.takeError()) << "\n";
    return Names;
  }
  forEachKernelNode(**DocOrErr,
...
KernelMeta extractKernelMeta(llvm::ArrayRef<uint8_t> ElfData,
  ...
  auto DocOrErr = COMGR::elf_helpers::getElfMetadataDocument(MB);
  if (!DocOrErr) {
    llvm::errs() << "transpiler: extractKernelMeta: "
                 << llvm::toString(DocOrErr.takeError()) << "\n";
    return Meta;
  }
  llvm::msgpack::Document &Doc = **DocOrErr;
...
std::string detectIsaFromElf(llvm::ArrayRef<uint8_t> ElfData) {
  auto IsaOrErr =
      COMGR::elf_helpers::getElfIsaNameFromHeader(toBufferRef(ElfData));
  if (!IsaOrErr) {
    // The ISA helper is also called from raise_cli on the malformed-input
    // path, so silently consume the error and let the caller fall back
    // to its filename heuristic.
    llvm::consumeError(IsaOrErr.takeError());
    return {};
  }
  return *IsaOrErr;
}
