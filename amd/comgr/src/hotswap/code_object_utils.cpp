...
#include "../co-utils/comgr-co-utils.h"
...
std::vector<std::string> listKernelNames(llvm::ArrayRef<uint8_t> ElfData) {
  ...
  auto DocOrErr =
      COMGR::co_utils::getElfMetadataDocument(toBufferRef(ElfData));
  if (!DocOrErr) {
...
KernelMeta extractKernelMeta(llvm::ArrayRef<uint8_t> ElfData,
  ...
  auto DocOrErr = COMGR::co_utils::getElfMetadataDocument(MB);
  if (!DocOrErr) {
...
std::string detectIsaFromElf(llvm::ArrayRef<uint8_t> ElfData) {
  auto IsaOrErr =
      COMGR::co_utils::getElfIsaNameFromHeader(toBufferRef(ElfData));
  if (!IsaOrErr) {
