...
struct RaiseContext {
  ...
  void recordReadFailure(llvm::Error Err) {
    ReadFailures.addError(std::move(Err));
  }
};
...
private:
  /// `llvm.amdgcn.update.dpp` has no FI operand. Records a refusal and returns
  /// true when Table-57 fetch-inactive is requested; callers must return their
