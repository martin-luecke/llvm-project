...
#define S(N) case CanonicalOp::N: return #N;
  switch (Op) {
    ...
    S(S_BUFFER_LOAD_B32) S(S_BUFFER_LOAD_B64) S(S_BUFFER_LOAD_B128)
    S(S_BUFFER_LOAD_B256) S(S_BUFFER_LOAD_B512)
    // SOPC
    ...
    S(V_MAX3_U32) S(V_MIN3_U32) S(V_MED3_I32)
    ...
    S(V_PERMLANE16_VAR_B32) S(V_PERMLANEX16_VAR_B32)
    // VOPC
    S(V_CMP) S(V_CMPX)
    // VOP3P
    ...
    // FLAT async global <- LDS store (gfx1250-only)
    S(GLOBAL_STORE_ASYNC_FROM_LDS_B8) S(GLOBAL_STORE_ASYNC_FROM_LDS_B32)
    S(GLOBAL_STORE_ASYNC_FROM_LDS_B64) S(GLOBAL_STORE_ASYNC_FROM_LDS_B128)
